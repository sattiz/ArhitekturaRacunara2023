...
## 78. Зачем нужна команда CALL?
Команда IZAZOVI (или INT - interrupt) в архитектуре компьютера используется для осознанного вызова исключения, или прерывания, в нормальном ходе выполнения программы. При выполнении команды IZAZOVI процессор прерывает выполнение программы и переходит в режим обработки исключений.

Команда IZAZOVI используется для вызова системных вызовов и других операций, требующих перехода в привилегированный режим работы. При выполнении команды IZAZOVI процессор переходит в привилегированный режим работы, а управление передается на соответствующий вектор прерывания, который указывает на адрес в памяти, где находится процедура обработки прерывания.

Процедура обработки прерывания может быть частью операционной системы или другого программного модуля, и ее задача состоит в обработке прерывания, выполнении соответствующих операций и возврате управления обратно в программу, которая была прервана.

Таким образом, команда IZAZOVI используется для осознанного вызова прерываний в нормальном ходе выполнения программы, что позволяет вызывать системные вызовы и другие операции, требующие перехода в привилегированный режим работы.

## 79. Na šta se oslanjaju sistemski pozivi? | На что опираются системные вызовы?
